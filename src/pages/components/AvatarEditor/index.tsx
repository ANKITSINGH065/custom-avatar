...
export default function AvatarEditor() {
  ...
  const generatePreview = async (flipped: boolean) => {
    const groups = await Promise.all(
      Object.keys(AvatarStyleCount).map(async (type) => {
        ...
        return `\n<g id="Custom_Art_System-avatar-${type}" ${
    ...
    if (festival) {
      ...
      groups.push(`\n<g id="Custom_Art_System-avatar-${festival}" ${
        flipped ? 'transform="scale(-1,1) translate(-1080, 0)"' : ''
      }>\n
    ${svgRaw.replace(/<svg.*(?=>)>/, '').replace('</svg>', '')}
  \n</g>\n`);
    ...
    const previewSvg =
      ...
      ${SVGFilter}
      <g id="Custom_Art_System-avatar" filter="url(#filter)">
        ${groups.join('\n\n')}
      </g>
      </svg>`
        .trim()
  ...
  const downloadAvatar = async () => {
    ...
    a.href = imageURL;
    a.download = `Custom_Art_System-avatar-${new Date().getTime()}.${imageType}`;
    a.click();
  };
